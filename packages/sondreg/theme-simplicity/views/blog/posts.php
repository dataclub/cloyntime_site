<?php $view->script('posts', 'blog:app/bundle/posts.js', 'vue') ?>
    ...
    <article class="uk-article">
        ...
        <p class="uk-article-meta">
          ...
          <a class="uk-display-block" href="<?= $view->url('@blog/id', ['id' => $post->id]) ?>"><img class="uk-border-circle" src="<?= $image ?>" alt="<?= $post->get('image.alt') ?>"></a>
          <?php endif ?>
          <?= $post->excerpt ?: $post->content ?>
            ...
            <div class="uk-width-medium-1-2 uk-container-center uk-text-center">
                ...
                <a class="uk-button uk-button-primary" href="<?= $view->url('@blog/id', ['id' => $post->id]) ?>"><?= __('Read more') ?></a>
                <?php endif ?>

                <?php if ($post->isCommentable() || $post->comment_count) : ?>
                <a class="uk-button uk-button-primary" href="<?= $view->url('@blog/id#comments', ['id' => $post->id]) ?>"><?= _c('{0} No comments|{1} %num% Comment|]1,Inf[ %num% Comments', $post->comment_count, ['%num%' => $post->comment_count]) ?></a>
                <?php endif ?>
            </div>
